...
import { writeLog, fileExists, removeDir, removeFile } from "./util";
import { spawnSync } from "node:child_process";
export function createTools(deps) {
    const { earsay, buffer, sse } = deps;
    async function ensureEarsay() {
        if (earsay.isRunning)
            return true;
        try {
    ...
    return {
        voice_start: tool({
            ...
            async execute() {
                ...
                if (!ok)
                    return JSON.stringify({ status: "failed", message: "earsay unavailable" });
        ...
        voice_pause: tool({
            ...
            args: {},
            async execute() {
                if (!await ensureEarsay())
                    return JSON.stringify({ status: "error", message: "server not running" });
                try {
        ...
        voice_subscribe: tool({
            ...
            async execute() {
                if (!await ensureEarsay())
                    return JSON.stringify({ status: "error", message: "server not running" });
                try {
                    sse.subscribe(earsay.baseUrl, (event) => buffer.onEvent(event.text || ""), (err) => writeLog(`SSE error: ${err.message}`), { chars: 30, timeout: 3000, fullchunk: true });
        ...
        voice_consume_all: tool({
            ...
            async execute() {
                try {
                    ...
                    if (pos === 0)
                        return JSON.stringify({ text: "", consumed: 0 });
